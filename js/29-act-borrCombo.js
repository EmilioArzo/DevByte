document.addEventListener('DOMContentLoaded', function() {
    ...
    function selectRow(row) {
        ...
        document.getElementById('bebida').value = cells[5].textContent;
        

        const img = cells[6].getElementsByTagName('img')[0];
        if (img) {
    ...
    function updateRow(row) {
        ...
        if (imagenFile) {
            ...
            reader.onload = function(event) {
                imagenUrl = event.target.result;
                updateRowInTable(row, id, nombre, descripcion, precio, categoria, bebida, imagenUrl);
            };
            reader.readAsDataURL(imagenFile);
        } else {
            updateRowInTable(row, id, nombre, descripcion, precio, categoria, bebida, imagenUrl);
    ...
    function updateRowInTable(row, id, nombre, descripcion, precio, categoria, bebida, imagenUrl) {
        const cells = row.getElementsByTagName('td');
